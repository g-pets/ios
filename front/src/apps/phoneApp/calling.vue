...
.view-calling.waiting
	.top-panel
		.calling-contact
			.number.dark-text-shadow +1 (234) 567-8900
	...
	.control-panel.glare
		.cell.mute
			...
			.label mute
		.cell.keypad
			glyph(name="keypad_rouded")
			.label keypad
		.cell.speaker
			glyph(name="speaker")
			.label speaker
		.cell.add-call
			glyph(name="plus_rounded")
			.label add call
		.cell.hold
			glyph(name="hold")
			.label hold
...
export default {
	...
	methods: {
		endCall() {
			this.$router.push({name: 'phoneApp_keypad'})
		}
	},
	setup() {
		document.title = "Calling | iOS"
		return {}
	}
}
</script>
...
.screenshot
	...
	top: -20px
	// left: 1px
	background-image: url("/screens/phoneApp_calling.jpg")
	width: 320px
	height: 480px
